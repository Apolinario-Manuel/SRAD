...
function App() {
  ...
  var columns = [
    ...
    {title: "Imagem", field: "img" ,
    render: rowData => <img src={`localhost/srad/uploads/${rowData.img}`} style={{width: 120, height: 90}}/> 
  },
  ...
  const handleRowDelete = (oldData, resolve) => {
    ...
    api.delete("/informacoes/"+oldData.id)
      .then(res => {
        console.log(res)
    ...
    <>
      ...
      <Grid container spacing={1}>
          ...
          <div style={{ margin: '6px', display: 'flex', justifyContent: 'flex-end' }}>
            <Link to="/app/blog/adicionar/novo">
              <Button
              ...
              actions={[
                {
                  icon: () => <Edit />,
                  tooltip: 'Editar',
                  onClick: (event, rowData) => { navigate(`/app/blog/adicionar/${rowData.id}`)} 
                }
              ]}
              editable={{
                onRowDelete: (oldData) =>
